...
| T04 | 56452 | ✅ | ✅ e2e | PASS (24/6/0) | ✅ e2e·docs/e2e + package.json(조건부 허용) | Playwright 30테스트 24통과/6스킵(DB·인증 의존, T04는 DB 적용 前 실행)/0실패. 회귀 0. S-AD1 테스트기대 정정(앱버그 아님). @playwright/test devDep 추가 |

## 검증 메트릭 (Phase 4) — R4 마감
- 완료율: **4/4 (100%)**
- 자가검증 PASS율: 4/4 (100%)
- 안티패턴 위반: 0 (전원 자기 쓰기영역만 수정 — T04 package.json은 조건부 허용)
...
- **E2E 실동작**(DB-ready 재실행): 28 passed / 1 failed(L-B4) / 1 skipped(AD1 인증). 직접 API로 댓글 작성(201)·댓글 추천 PATCH(like_count 0→1→0) 정상 확인 → **L-B4 실패는 앱버그 아님, T04 E2E spec 신뢰성 이슈(R5)**. 게시글 추천/비추·댓글 작성·댓글 추천 백엔드+wiring 전부 실동작 검증 완료.
