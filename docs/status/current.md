...
| 항목 | 값 |
|------|-----|
| **마지막 세션** | 2026-05-30 (세션 40 — **R13 지휘부 — display-rollout 마감 + 배포 경로 정상화**: 시세 전역구독·watchlist 후속·계정 드롭다운·dev500 Tailwind 핫픽스·레포 Public 전환·Vercel 자동배포 확인) |
...
| **브랜치** | main |
| **빌드 상태** | ✅ `npx tsc --noEmit` 0 / `npm run build` ✓ Compiled (CSS 경고 0·핫픽스 후) / `npx eslint`(R13 11파일) error 0 / dev `/`·`/watchlist`·`/coin/btc`·`/settings` HTTP 200. 라이브 `coinchart.vercel.app` 200(Vercel 자동배포). |
| **마지막 커밋** | `b9fd654`(세션40 — 중복 release 워크플로우 비활성화). **직전**: `a90a059`(R13 통합 마감 — 시세구독·watchlist 후속·계정 드롭다운·dev500 핫픽스). origin/main push. (cs 커밋 후속) |
...
- **R13 후속(R14 후보)**: watchlist 회원 sync 런타임 스모크(실 로그인 1회 — T-B 절차서) · **DEPLOYMENT_RUNBOOK 정정**(GitHub Release 게이트 가정 ↔ 실제 Vercel Git 자동배포 불일치) · 시세 구독 잔여(FngGauge·HotIssue 트렌드색·KimchiPremium·StockTicker 다통화) · `daily-cron` Public 전환 후 정상작동 확인
- **배포 구조 확정(세션40)**: 실제 배포 = **Vercel Git 자동배포**(main push 자동, `coinchart.vercel.app`). GitHub Actions `release-*` 파이프라인은 VERCEL secret 미설정 중복 → `workflow_dispatch` 비활성화. 레포 **Public**(Actions 무료). [[private-actions-billing-vercel-git-deploy]]
