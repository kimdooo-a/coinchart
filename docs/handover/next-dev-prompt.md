...
- ~~**TradingView 차트**: 다크 옵션 그대로라 라이트 환경에 어울리지 않음. `lib/chart/theme.ts`로 분리 필요.~~ → **세션 9(T08) 완료** — `lib/chart/theme.ts` 헬퍼 신규. **세션 19(T11)에서 page/component 라이트화는 완료했으나** 차트 생성 호출(`createChart`)이 `components/Chart/*`·`DetailedChart`·`hero-chart`에 있어 차트 옵션 자체는 후속 라운드 적용 대기 (T10·T11 영역에는 차트 생성 호출 부재).
