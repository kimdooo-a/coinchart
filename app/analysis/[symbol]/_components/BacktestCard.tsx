...
export function BacktestCard({ analysisResult, userTier, lang }: BacktestCardProps) {
    if (!analysisResult.backtest) return null
        ...
        <Card className="bg-surface-container-lowest border-outline-variant relative overflow-hidden">
            ...
            {analysisResult.backtest.status === 'insufficient' ? (
                ...
                <>
                    ...
                    {/* Pro Lock Overlay */}
                    <div className="absolute inset-0 bg-on-surface/20 flex flex-col items-center justify-center gap-4">
                        <Badge variant="outline" className="border-yellow-500 text-yellow-500">
                            PRO
                        </Badge>
                        <Link href="/pricing">
