...
export const COIN_INFO: Record<string, { titleEn: string; titleKo: string; descEn: string; descKo: string }> = {
    BTC: {
...
export const COIN_HISTORY: Record<string, HistoryEvent[]> = {
    BTC: [
        {
            year: 2008, month: '10',
    ...
    BCH: [
        {
            ...
            detailsEn: 'Culminating years of heated debate over Bitcoin\'s scalability, the "Big Blockers" (led by Roger Ver and Jihan Wu) executed a hard fork to create Bitcoin Cash. They increased the block size limit from 1MB to 8MB, prioritizing on-chain scaling to keep transactions cheap and fast, embodying the vision of "Peer-to-Peer Electronic Cash" described in Satoshi\'s original whitepaper.',
            detailsKo: '비트코인의 확장성을 둘러싼 수년간의 격렬한 논쟁("블록 크기 전쟁") 끝에, 로저 버와 우지한이 이끄는 "빅 블로커" 진영이 하드포크를 단행하여 비트코인 캐시를 창시했습니다. 그들은 블록 크기 제한을 1MB에서 8MB로 늘려 온체인 확장을 최우선으로 삼았으며, 이를 통해 사토시 나카모토의 원본 백서에 묘사된 "P2P 전자 화폐"의 비전을 실현하고자 했습니다. 이는 수수료를 낮게 유지하고 거래 속도를 높이는 데 중점을 두었습니다.',
            type: 'milestone'
        },
        {
            year: 2017, month: '11',
            ...
            detailsEn: 'Rumored to be orchestrated by key BCH proponents, "Operation Dragon Slayer" aimed to flip the market cap of BCH and BTC (The Flippening). The plan involved a massive pump of BCH prices combined with a coordinated shift of mining power to "starve" the Bitcoin legacy chain. This caused BTC transaction fees to spike to over $50 and confirmed transactions to grind to a halt. While BTC survived the attack, it remains one of the most dramatic moments in crypto history.',
            detailsKo: 'BCH의 주요 지지자들이 주도한 것으로 알려진 "드래곤 슬레이어 작전"은 BCH가 BTC의 시가총액을 추월(Flippening)하려는 대담한 시도였습니다. 이 계획은 BCH 가격을 인위적으로 폭등시킴과 동시에 채굴 해시레이트를 BCH로 대거 이동시켜 비트코인 네트워크를 "굶겨(Starve)" 고사시키는 것이었습니다. 이로 인해 BTC 전송 수수료가 50달러를 넘고 거래가 멈추는 등 위기가 닥쳤으나, BTC는 결국 살아남았습니다. 암호화폐 역사상 가장 드라마틱한 사건 중 하나입니다.',
            type: 'drama'
        },
        {
            year: 2017, month: '12',
            ...
            detailsEn: 'Coinbase surprisingly enabled BCH trading earlier than expected, causing the price to skyrocket instantly to nearly $4,000 on some exchanges. Accusations of insider trading emerged as the price pumped significantly before the public announcement, leading to internal investigations and lawsuits against the exchange. This peak marked the height of the BCH mania.',
            detailsKo: '미국 최대 거래소 코인베이스가 예고 없이 기습적으로 BCH 거래를 지원하자 가격이 순식간에 폭등하여 일부 거래소에서는 4,000달러에 육박했습니다. 공식 발표 전부터 가격이 급등한 정황이 포착되어 내부자 거래 의혹이 제기되었고, 이는 조사와 소송으로 이어졌습니다. 이 시기는 비트코인 캐시 열풍의 정점이었습니다.',
            type: 'market'
        },
        {
            year: 2018, month: '05',
            descEn: 'Block Size upgrade to 32MB. Reactivating Op-Codes.',
            descKo: '블록 크기 32MB로 확장 및 스마트 컨트랙트 기초(Op-Codes) 복원.',
            detailsEn: 'BCH upgraded its network to quadruple the block size limit to 32MB, allowing for even more throughput. Additionally, it reactivated several "Op-Codes" (operation codes) that had been disabled in Bitcoin, paving the way for simple smart contracts and tokenization features on the network.',
            detailsKo: 'BCH는 네트워크 업그레이드를 통해 블록 크기 제한을 32MB로 4배 확장하여 처리량을 대폭 늘렸습니다. 또한, 비트코인에서 비활성화되었던 여러 "Op-Codes"(명령어 코드)를 다시 활성화하여 네트워크 상에서 간단한 스마트 컨트랙트와 토큰화 기능을 구현할 수 있는 길을 열었습니다.',
            type: 'tech'
        },
        {
            year: 2018, month: '11',
            ...
            detailsEn: 'A bitter ideological split turned into a digital war between Roger Ver\'s Bitcoin ABC (preserving BCH) and Craig Wright\'s Bitcoin SV (BSV). Both sides burned millions of dollars in electricity to attack each other\'s chains. The uncertainty and chaos triggered a massive sell-off across the entire global crypto market, sending BTC plummeting to its cycle low of $3,000.',
            detailsKo: '이념적 갈등이 격화되어 로저 버의 Bitcoin ABC(BCH 수호)와 크레이그 라이트의 Bitcoin SV(BSV) 간에 디지털 전쟁이 발발했습니다. 양측은 서로의 체인을 공격하고 방어하기 위해 수백만 달러 규모의 전력을 쏟아부었습니다. 이 혼란과 공포는 글로벌 크립토 시장 전체의 투매를 촉발했고, 결국 비트코인(BTC)을 3,000달러라는 사이클 최저점까지 끌어내리는 원인이 되었습니다.',
            type: 'drama'
        },
        {
            year: 2020, month: '11',
            descEn: 'Block Tax Revolt. BCH rejects central funding tax, splits from ABC.',
            descKo: '블록 보상 세금("IFP") 거부. 커뮤니티가 개발 독재를 막아냄.',
            detailsEn: 'Lead developer Amaury Séchet proposed a controversial "Infrastructure Funding Plan" (IFP) that would tax 8% of mining rewards to fund his development team. The community and miners viewed this as a centralization attack and revolted. They successfully forked Amaury out (who went on to form eCash/XEC), ensuring BCH remained decentralized and community-driven.',
            detailsKo: 'BCH의 수석 개발자 아마우리 세쳇이 채굴 보상의 8%를 개발팀 자금으로 걷는 "인프라 자금 조달 계획(IFP)"을 제안했습니다. 커뮤니티와 채굴자들은 이를 중앙화된 세금으로 간주하여 강력히 반발했습니다. 결국 커뮤니티는 아마우리를 축출(이후 eCash/XEC로 분리)하고 BCH의 탈중앙성을 지켜냈습니다. 이는 개발 독재를 막아낸 승리로 기록됩니다.',
            type: 'tech'
        },
        {
            year: 2023, month: '05',
            ...
            detailsEn: 'The historic CashTokens upgrade enabled native token issuance (fungible & NFTs) and advanced smart contracts directly on Bitcoin Cash. This unlocked the potential for Decentralized Exchanges (DEXs) and DeFi applications, mimicking Ethereum\'s utility while maintaining Bitcoin\'s security model and low fees.',
            detailsKo: '역사적인 캐시토큰(CashTokens) 업그레이드를 통해 비트코인 캐시에서 직접 토큰(대체 가능 및 NFT)을 발행하고 고급 스마트 컨트랙트를 실행할 수 있게 되었습니다. 이는 비트코인의 보안 모델과 저렴한 수수료를 유지하면서도 이더리움과 같은 디파이(DeFi) 및 탈중앙화 거래소(DEX) 생태계를 구축할 수 있는 잠재력을 열어주었습니다.',
            type: 'tech'
        },
        {
            year: 2024, month: '04',
            ...
            detailsEn: 'Bitcoin Cash underwent its second halving, reducing the block reward from 6.25 to 3.125 BCH. While the price rallied leading up to the event, it also reignited discussions about the long-term sustainability of the network\'s security budget relying solely on transaction fees in the future.',
            detailsKo: '비트코인 캐시는 두 번째 반감기를 맞이하여 블록 보상이 6.25 BCH에서 3.125 BCH로 줄어들었습니다. 반감기를 앞두고 가격이 상승했지만, 동시에 향후 거래 수수료만으로 네트워크 보안을 유지할 수 있을지에 대한 장기적인 지속 가능성 논의가 다시 점화되었습니다.',
            type: 'milestone'
        },
    ],
    DOGE: [
