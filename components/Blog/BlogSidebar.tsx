...
export default function BlogSidebar({ categories, tags, recentPosts }: BlogSidebarProps) {
  ...
  return (
    <aside className="space-y-8">
      {/* 카테고리 */}
      <div className="p-5 border border-outline-variant rounded-xl bg-surface-container">
          ...
          <div className="flex flex-wrap gap-1.5">
            {tags.slice(0, 20).map((tag) => (
              <Link
                ...
                className="px-2.5 py-0.5 text-xs text-on-surface-variant bg-surface-container border border-outline-variant rounded-full hover:text-on-surface hover:border-outline transition-colors"
              >
                #{tag.name}
              </Link>
            ))}
          ...
          <ul className="space-y-3">
            {recentPosts.slice(0, 5).map((post) => (
              <li key={post.id}>
                <Link
                  href={`/blog/${post.slug}`}
                  className="block text-sm text-on-surface-variant hover:text-on-surface transition-colors line-clamp-2"
                >
                  {post.title}
                </Link>
